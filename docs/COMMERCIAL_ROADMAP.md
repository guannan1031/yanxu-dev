...
| v0.16 私有服务 Alpha | API、PostgreSQL、Worker、组织与最小 RBAC；GitHub App/Webhook、去重与撤权；Docker Compose 私有部署 | 重启后状态可恢复；事件不产生错误状态；撤权后停止访问；备份恢复有测试证据 |
...
下一项代码优先级是 **v0.16 私有服务 Alpha 的最小纵向切片**。先建立 API、PostgreSQL 持久化和单组织权限，再接 GitHub App/Webhook；不直接建设多租户 SaaS。
...
| When | v0.15 已完成；下一步开发私有服务最小切片并启动四周试点验证；SaaS 由付费续约触发 |
...
> 我开源了 Yanxu Dev v0.15.0，用任务合同和团队 Policy 限制 AI 修改范围，在隔离副本中生成代码并自动测试；团队工作空间可只读同步多个项目的真实 PR/CI 状态，并将人工复核、配对提效证据和 Policy 指纹汇总到私有看板。项目已在 Linux 和 Windows CI 验证，下一阶段是私有服务最小切片和真实付费试点。
