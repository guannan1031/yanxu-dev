...
| 阶段 | 计划交付 | 退出条件 |
| --- | --- | --- |
| v0.14 试点证据包 | `team export` 导出可交付证据包；展示每个仓库的 Policy、测试、PR/CI 与测量状态；Windows 干净环境安装检查 | 别人按文档完成安装；一个试点证据包可离线审查；不包含源码和凭据 |
| v0.15 团队 GitHub 接入 | 只读 GitHub App/Webhook 试验；多 PR/CI 事件去重；仓库撤权处理 | PR 和 CI 绑定实际 commit；重复/乱序事件不产生错误状态；撤权后停止访问 |
| v0.16 私有服务 Alpha | API、PostgreSQL、Worker、组织与最小 RBAC；Docker Compose 私有部署 | 重启后状态可恢复；团队间数据隔离；备份恢复有测试证据 |
...
下一项代码优先级是 **v0.14 试点证据包**。它直接提高可交付性和销售演示质量，也为后续团队服务提供统一验收材料。
...
| When | 先完成 v0.14，再用四周试点验证；SaaS 由付费续约触发 |
...
> 我开源了 Yanxu Dev v0.13.0，用任务合同和团队 Policy 限制 AI 修改范围，在隔离副本中生成代码并自动测试，把 PR/CI、人工复核和真实配对提效证据汇总到私有团队看板。项目已在 Linux 和 Windows CI 验证，下一阶段是试点证据包和真实团队付费验证。
