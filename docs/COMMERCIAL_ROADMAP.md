...
日期：2026-09-15。当前产品版本：v0.20.0（发布候选）。
...
| v0.20.0 试点验收证据（发布候选） | 组织隔离的验收清单、客户确认记录、支持分钟和扩展试点 ZIP | owner/viewer 权限、证据必填、组织隔离、浏览器和 manifest 哈希通过 |
| 外部付费试点 | 用 v0.20 工作台执行四周试点，客户签字文件仅作为证据引用 | 1 个外部团队完成约定任务；验收结果、支持投入和费用可核对 |
| 托管团队版 | 只有持续付费需求成立后建设 SSO、托管隔离、订阅和 SLA | 至少 2 个试点完成、1 个继续付费，并确认客户确实需要托管 |

下一项产品优先级是 **外部团队试点准备与真实 UAT**。私有 API、PostgreSQL、组织权限、GitHub 事件队列、团队网页、实际成本、审计、试点 ZIP、备份、恢复、安装预检、验收清单和支持投入已实现；下一步准备报价、数据处理边界和客户签字模板，并用外部团队的真实记录判断是否续费。合同或签字文件不上传平台，只保存客户批准的证据引用。
...
| When | v0.16–v0.19.0 已发布，v0.20.0 进入发布候选；下一阶段启动四周外部试点；SaaS 由付费续约触发 |
...
> 我开发了 Yanxu Dev v0.20.0，用任务合同和团队 Policy 限制 AI 修改范围，在隔离副本中生成代码并自动测试；团队工作空间同步多个项目的 PR/CI 状态，并将标准化证据保存到组织隔离的 PostgreSQL 私有服务。浏览器工作台记录验收标准、客户确认、支持投入和分币种成本，导出带哈希 manifest 的试点 ZIP；跨平台命令提供脱敏安装预检、备份和单事务恢复。系统区分内部通过、客户确认记录和外部签字，未完成真实配对测量时明确标记 `NOT_MEASURED`；下一阶段是真实团队试点。
