...
日期：2026-09-15。当前产品版本：v1.0.0（已发布）。
...
| v1.0.0 可收费试点包（已发布） | 打印验收报告、证据指纹、三档固定范围报价、UAT 与交接清单 | PostgreSQL、浏览器、Windows/Linux CI 和发布资产通过；报告不冒充电子签名或提效证明 |
...
当前产品的下一项优先级是 **外部团队真实 UAT 与付费验证**。私有 API、PostgreSQL、组织权限、GitHub 事件队列、团队网页、实际成本、审计、试点 ZIP、打印报告、备份恢复、安装预检、报价和交接模板已经具备；下一步用外部团队的真实记录判断是否验收与续费。合同或签字文件不上传平台，只保存客户批准的证据引用。
...
| When | v0.16–v1.0.0 已发布；随后启动四周外部试点；SaaS 由付费续约触发 |
...
> 我开发了 Yanxu Dev v1.0.0，用任务合同和团队 Policy 限制 AI 修改范围，在隔离副本中生成代码并自动测试；团队工作空间同步多个项目的 PR/CI 状态，并将标准化证据保存到组织隔离的 PostgreSQL 私有服务。浏览器工作台记录验收标准、客户确认、支持投入和分币种成本，导出带哈希 manifest 的试点 ZIP 与可打印验收报告；跨平台命令提供脱敏安装预检、备份和单事务恢复。系统区分内部通过、客户确认记录和外部签字，未完成真实配对测量时明确标记 `NOT_MEASURED`；下一阶段是真实团队付费试点。
