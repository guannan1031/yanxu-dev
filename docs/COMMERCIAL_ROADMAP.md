...
| v0.16.1 GitHub 事件接入 | GitHub App/Webhook、Worker、delivery 去重、安装撤权 | 重放事件不产生重复状态；撤权后停止访问；失败进入可审计重试队列 |
...
下一项代码优先级是 **v0.16.1 GitHub 事件接入**。私有 API、PostgreSQL 和组织权限的纵向切片已经实现；下一步接 GitHub App/Webhook 和 Worker，仍不直接建设多租户 SaaS。
...
| When | v0.16 私有服务最小切片已完成；随后完成 GitHub 事件接入并启动四周试点验证；SaaS 由付费续约触发 |
...
> 我开发了 Yanxu Dev v0.16，用任务合同和团队 Policy 限制 AI 修改范围，在隔离副本中生成代码并自动测试；团队工作空间可同步多个项目的真实 PR/CI 状态，并将标准化证据保存到组织隔离的 PostgreSQL 私有服务。服务包含 owner/viewer、哈希令牌、幂等写入、审计和 Docker 部署，并完成跨组织隔离及备份恢复测试。下一阶段是 GitHub App/Webhook 和真实付费试点。
