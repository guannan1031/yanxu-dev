...
日期：2026-09-15。当前产品版本：v1.0.0（候选版，发布证据待 CI 完成）。
...
| v0.17.1 成本与试点包（已发布） | 按币种记录模型/CI/基础设施/支持实际成本；导出带哈希 manifest 的 ZIP | 精确金额、owner 写权限、组织隔离、浏览器下载和哈希复算通过；提效状态固定为 `NOT_MEASURED` |
| v0.18.0 运维恢复（已发布） | 跨平台 Docker Compose 备份、manifest 校验、单事务恢复、升级故障手册 | 文件篡改在停服前拒绝；真实 Docker 恢复后数据回到备份时点且服务自动重启 |
| v0.19.0 安装预检（已发布） | 非覆盖随机 `.env`、11 项脱敏 preflight、Windows/macOS/Linux 统一命令 | 空卷 Docker 启动、版本、HttpOnly 登录和无 Token 回显通过 |
| v0.20.0 试点验收证据（已发布） | 组织隔离的验收清单、客户确认记录、支持分钟和扩展试点 ZIP | owner/viewer 权限、证据必填、组织隔离、浏览器和 manifest 哈希通过 |
| v1.0.0 可收费试点包（候选版） | 打印验收报告、证据指纹、三档固定范围报价、UAT 与交接清单 | PostgreSQL、浏览器、Windows/Linux CI 和发布资产通过；报告不冒充电子签名或提效证明 |
| 外部付费试点 | 用 v1.0 工作台执行四周试点，客户签字文件仅作为证据引用 | 1 个外部团队完成约定任务；验收结果、支持投入和费用可核对 |
| 托管团队版 | 只有持续付费需求成立后建设 SSO、托管隔离、订阅和 SLA | 至少 2 个试点完成、1 个继续付费，并确认客户确实需要托管 |

v1.0 代码与材料通过发布验收后的下一项优先级是 **外部团队真实 UAT 与付费验证**。私有 API、PostgreSQL、组织权限、GitHub 事件队列、团队网页、实际成本、审计、试点 ZIP、打印报告、备份恢复、安装预检、报价和交接模板已经具备；下一步用外部团队的真实记录判断是否验收与续费。合同或签字文件不上传平台，只保存客户批准的证据引用。
...
| When | v0.16–v0.20.0 已发布，v1.0.0 正在发布验收；随后启动四周外部试点；SaaS 由付费续约触发 |
...
> 我开发了 Yanxu Dev v1.0.0，用任务合同和团队 Policy 限制 AI 修改范围，在隔离副本中生成代码并自动测试；团队工作空间同步多个项目的 PR/CI 状态，并将标准化证据保存到组织隔离的 PostgreSQL 私有服务。浏览器工作台记录验收标准、客户确认、支持投入和分币种成本，导出带哈希 manifest 的试点 ZIP与可打印验收报告；跨平台命令提供脱敏安装预检、备份和单事务恢复。系统区分内部通过、客户确认记录和外部签字，未完成真实配对测量时明确标记 `NOT_MEASURED`；下一阶段是真实团队付费试点。
