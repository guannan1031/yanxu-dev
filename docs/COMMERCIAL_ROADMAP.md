...
日期：2026-09-14。当前产品版本：v0.16.1（发布候选）。
...
| v0.16.1 GitHub 事件接入（已实现，待发布） | GitHub App/Webhook、Worker、delivery 去重、安装撤权 | 错签名拒绝；重复事件只处理一次；撤权后停止处理；队列与审计可查询 |
| v0.17 付费试点版 | 接入向导、审计导出、用量成本、升级和故障手册 | 1 个外部团队完成约定任务；验收结果、支持投入和费用可核对 |
| 托管团队版 | 只有持续付费需求成立后建设 SSO、托管隔离、订阅和 SLA | 至少 2 个试点完成、1 个继续付费，并确认客户确实需要托管 |

下一项代码优先级是 **v0.17 付费试点界面与验收包**。私有 API、PostgreSQL、组织权限和 GitHub 事件队列已经实现；下一步提供团队网页、用量成本、接入向导和审计导出。
...
| When | v0.16 私有服务与 v0.16.1 GitHub 事件接入已实现；下一阶段完成试点界面与验收包并启动四周试点；SaaS 由付费续约触发 |
