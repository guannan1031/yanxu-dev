...
日期：2026-09-15。当前产品版本：v0.19.0（发布候选）。
...
| v0.19.0 安装预检（发布候选） | 非覆盖随机 `.env`、11 项脱敏 preflight、Windows/macOS/Linux 统一命令 | 空卷 Docker 启动、版本、HttpOnly 登录和无 Token 回显通过 |
| v0.19.x 付费试点验收 | 试点验收流程、支持记录和签字模板 | 1 个外部团队完成约定任务；验收结果、支持投入和费用可核对 |
| 托管团队版 | 只有持续付费需求成立后建设 SSO、托管隔离、订阅和 SLA | 至少 2 个试点完成、1 个继续付费，并确认客户确实需要托管 |

下一项代码优先级是 **v0.19.x 试点验收与支持记录**。私有 API、PostgreSQL、组织权限、GitHub 事件队列、团队网页、实际成本、审计、试点 ZIP、备份、恢复和安装预检已实现；下一步生成客户可签字的验收清单和支持投入记录，再启动外部团队试点。
...
| When | v0.16–v0.18.0 已发布，v0.19.0 安装预检进入发布候选；下一阶段完成验收模板并启动四周试点；SaaS 由付费续约触发 |
...
> 我开发了 Yanxu Dev v0.19.0，用任务合同和团队 Policy 限制 AI 修改范围，在隔离副本中生成代码并自动测试；团队工作空间可同步多个项目的真实 PR/CI 状态，并将标准化证据保存到组织隔离的 PostgreSQL 私有服务。服务包含 owner/viewer、哈希令牌、审计、GitHub Webhook 队列和 Docker 部署；浏览器工作台记录分币种成本并导出带哈希 manifest 的试点 ZIP。跨平台安装命令随机生成私密配置并输出 11 项脱敏预检，运维命令提供备份、篡改校验和单事务恢复。未完成真实配对测量时平台明确标记 `NOT_MEASURED`；下一阶段是真实团队试点。
