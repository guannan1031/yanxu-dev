...
下一项代码优先级是 **v0.15 团队 GitHub 只读接入**。它把多个项目的真实 PR/CI 状态汇入团队证据层，同时保留人工合并和现有分支保护。
...
| When | v0.14 已完成；下一步开发只读 GitHub 接入并启动四周试点验证；SaaS 由付费续约触发 |
...
> 我开源了 Yanxu Dev v0.14.0，用任务合同和团队 Policy 限制 AI 修改范围，在隔离副本中生成代码并自动测试，把 PR/CI、人工复核和真实配对提效证据汇总到私有团队看板，并可导出带哈希的脱敏试点证据包。项目已在 Linux 和 Windows CI 验证，下一阶段是团队 GitHub 只读接入和真实付费试点。
