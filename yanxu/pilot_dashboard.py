...
def build_acceptance_report(store: PostgresStore, github_store,
                            principal: Principal) -> tuple[str, str]:
    summary = build_summary(store, github_store, principal)
    evidence = {
        "schema_version": 1,
        "kind": "yanxu.pilot_acceptance_report",
        ...
        "efficiency_claim_status": "NOT_MEASURED",
        "claim_boundary": "Customer confirmation is owner-recorded; the printable report is not an electronic signature or independent identity verification.",
    }
    ...
    acceptance_rows = "".join(
        "<tr>" + "".join(f"<td>{_escape(value)}</td>" for value in (
            item["criterion"], ACCEPTANCE_STATUS_LABELS[item["status"]],
            item.get("evidence_ref") or "—", "是" if item["customer_confirmed"] else "否",
        )) + "</tr>" for item in summary["acceptance"]
    ) or '<tr><td colspan="4">尚未配置验收标准。</td></tr>'
    support_rows = "".join(
        "<tr>" + "".join(f"<td>{_escape(value)}</td>" for value in (
            SUPPORT_CATEGORY_LABELS[item["category"]], item["minutes"],
            item["evidence_ref"], item["created_at"],
        )) + "</tr>" for item in summary["support"]
    ) or '<tr><td colspan="4">尚未记录支持投入。</td></tr>'
    cost_rows = "".join(
        "<tr>" + "".join(f"<td>{_escape(value)}</td>" for value in (
            item["currency"], COST_CATEGORY_LABELS[item["category"]],
            item["amount"], item["records"],
        )) + "</tr>" for item in summary["cost_summary"]
    ) or '<tr><td colspan="4">尚未记录实际成本。</td></tr>'
    acceptance = summary["acceptance_summary"]
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>研序试点验收报告</title><style>body{{margin:0;background:#eef4f1;color:#183b33;font:14px/1.6 system-ui,-apple-system,"PingFang SC",sans-serif}}main{{max-width:900px;margin:24px auto;background:white;padding:42px;border:1px solid #d8e7e0}}h1{{margin:0;font-size:30px}}h2{{margin:28px 0 8px;font-size:18px;border-bottom:2px solid #1a7b63;padding-bottom:6px}}.meta,.boundary{{color:#607c74}}.summary{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:24px 0}}.card{{border:1px solid #d8e7e0;border-radius:8px;padding:12px}}.value{{font-size:20px;font-weight:800}}table{{width:100%;border-collapse:collapse}}th,td{{padding:8px;text-align:left;border-bottom:1px solid #e2ece8;overflow-wrap:anywhere}}th{{color:#607c74}}.boundary{{background:#f2f7f5;padding:12px;border-left:4px solid #1a7b63}}.signatures{{display:grid;grid-template-columns:1fr 1fr;gap:42px;margin-top:38px}}.line{{border-bottom:1px solid #183b33;height:38px}}.fingerprint{{font:11px/1.5 ui-monospace,monospace;overflow-wrap:anywhere}}@media(max-width:650px){{main{{margin:0;padding:22px}}.summary,.signatures{{grid-template-columns:1fr 1fr}}}}@media print{{body{{background:white}}main{{border:0;margin:0;max-width:none}}}}</style></head><body><main><div class="meta">YANXU DEV v1.0.0 · 私有试点交付</div><h1>试点验收报告</h1><p class="meta">组织：{_escape(principal.organization_slug)}　生成时间：{_escape(summary['created_at'])}</p><div class="summary"><div class="card"><div class="value">{_escape(ACCEPTANCE_STATUS_LABELS[acceptance['status']])}</div><div>总体状态</div></div><div class="card"><div class="value">{acceptance['passed']} / {acceptance['total']}</div><div>通过项</div></div><div class="card"><div class="value">{acceptance['customer_confirmed']}</div><div>客户确认记录</div></div><div class="card"><div class="value">{summary['support_summary']['minutes']}</div><div>支持分钟</div></div></div><h2>验收清单</h2><table><thead><tr><th>标准</th><th>状态</th><th>证据引用</th><th>客户确认</th></tr></thead><tbody>{acceptance_rows}</tbody></table><h2>支持投入</h2><table><thead><tr><th>类型</th><th>分钟</th><th>证据引用</th><th>记录时间</th></tr></thead><tbody>{support_rows}</tbody></table><h2>实际成本汇总</h2><table><thead><tr><th>币种</th><th>类型</th><th>金额</th><th>记录数</th></tr></thead><tbody>{cost_rows}</tbody></table><h2>声明边界</h2><p class="boundary">本报告来自组织 owner 录入的验收、客户确认、支持和成本记录。它不构成电子签名、客户身份独立核验、付款证明或提效百分比证明；提效状态为 NOT_MEASURED。正式验收应由双方在导出后按合同流程签署。</p><div class="signatures"><div><div class="line"></div><div>客户代表 / 日期</div></div><div><div class="line"></div><div>交付方 / 日期</div></div></div><h2>证据指纹</h2><p class="fingerprint">SHA-256: {_escape(fingerprint)}</p></main></body></html>""", fingerprint
